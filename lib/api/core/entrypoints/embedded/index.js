...
const
  ...
  fs = require('fs'),
  uWebSockets = require('uWebSockets.js'),
  {
    models: {
      RequestContext
    },
...
class EmbeddedEntryPoint extends EntryPoint {
  ...
  init () {
    // We need to verify the port ourselves: if not a number, µWebSockets
    // will open a named pipe instead of an actual network port, making the
    // kuzzle host unreachable from the outside without any error
    assert(
      Number.isInteger(this.config.port),
      `Invalid network port number: ${this.config.port}`);
  ...
  */
  ...
  _createHttpServer() {
    ...
    return new Bluebird((resolve, reject) => {
      server.listen(this.config.port, token => {
        if (token) {
          resolve(server);
        } else {
          reject(new KuzzleInternalError(`Failed to listen to HTTP port ${this.config.port}`));
        }
      });
    });
  }
